...
# gets weather data from an open source website 
...
# Sets boundaries for the map & loads the grid
...
# Random forest regressor (AI) prediction code
...
folium.raster_layers.ImageOverlay( #adds the heatmap onto the map
    image=image_url,
...
# map interactions (detects clicks on the map)
...
with col2:
    ...
    with st.container(border=True):
        ...
        if "clicked_data" in st.session_state and st.session_state.clicked_data:
            ...
            st.info("Click anywhere inside the map area to see the temperature at that point.")

    st.markdown("#### Misc")
    st.markdown(f"""
    * **Live Weather Feed Status:** `Connected`
    * **Target Forecast Horizon:** `+{forecast_hour} Hour(s)`
    * **Peak Temperature:** **{np.max(nrt_final_heatmap):.1f}°C**
    * **Minimum Temperature:** **{np.min(nrt_final_heatmap):.1f}°C**
    """)
